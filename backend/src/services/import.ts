...
import { db, budgetItems, budgetYears } from '../db/index.js';
...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  ...
  'alimentation': ['migros', 'coop', 'lidl', 'aldi', 'denner', 'spar', 'volg', 'manor food', 'aligro', 'carrefour', 'leclerc', 'auchan', 'intermarché', 'casino', 'franprix', 'monoprix', 'picard', 'boulangerie', 'boucherie', 'primeur'],
  'courses': ['migros', 'coop', 'lidl', 'aldi', 'denner', 'spar', 'volg', 'supermarché', 'épicerie', 'grocery'],
  
  // Restaurants / Dining
  'restaurant': ['restaurant', 'resto', 'pizzeria', 'brasserie', 'bistro', 'café', 'starbucks', 'mcdonald', 'burger king', 'subway', 'kfc', 'kebab', 'sushi', 'thai', 'chinois', 'italien', 'take away', 'takeaway', 'uber eats', 'deliveroo', 'just eat', 'smood'],
  'sorties': ['restaurant', 'bar', 'pub', 'club', 'cinema', 'cinéma', 'concert', 'spectacle', 'théâtre', 'musée'],
  
  // Transport
  'transport': ['sbb', 'cff', 'ffs', 'tpg', 'tl', 'bls', 'ratp', 'sncf', 'train', 'tram', 'bus', 'metro', 'métro', 'uber', 'taxi', 'bolt', 'lyft', 'parking', 'parcmètre', 'essence', 'shell', 'bp', 'esso', 'migrol', 'agrola', 'péage', 'autoroute', 'vignette'],
  'voiture': ['essence', 'shell', 'bp', 'esso', 'migrol', 'agrola', 'garage', 'pneu', 'révision', 'entretien auto', 'parking', 'tcs', 'assurance auto'],
  ...
  // Telecom / Internet
  'téléphone': ['swisscom', 'sunrise', 'salt', 'wingo', 'yallo', 'lebara', 'lycamobile', 'orange', 'sfr', 'bouygues', 'free mobile', 'apple', 'samsung', 'mobile'],
  ...
  // Health
  'santé': ['pharmacie', 'pharmacy', 'médecin', 'docteur', 'hôpital', 'clinique', 'dentiste', 'opticien', 'optique', 'lunettes', 'lentilles', 'laboratoire', 'radiologie', 'kiné', 'physio', 'ostéo', 'css', 'swica', 'helsana', 'sanitas', 'visana', 'concordia', 'groupe mutuel', 'assura', 'atupri'],
  'assurance maladie': ['css', 'swica', 'helsana', 'sanitas', 'visana', 'concordia', 'groupe mutuel', 'assura', 'atupri', 'mutuelle'],
  
  // Shopping
  'vêtements': ['h&m', 'zara', 'mango', 'uniqlo', 'c&a', 'manor', 'globus', 'zalando', 'la redoute', 'asos', 'primark', 'décathlon', 'intersport', 'ochsner'],
  'shopping': ['amazon', 'galaxus', 'digitec', 'fnac', 'darty', 'ikea', 'conforama', 'manor', 'globus', 'jelmoli', 'wish', 'aliexpress', 'temu', 'shein'],
  
  // Insurance
  'assurance': ['axa', 'allianz', 'zurich', 'helvetia', 'mobilière', 'mobiliar', 'bâloise', 'generali', 'vaudoise', 'maif', 'macif', 'matmut', 'groupama'],
  
  // Bank / Finance
  'banque': ['ubs', 'credit suisse', 'raiffeisen', 'bcv', 'bcge', 'postfinance', 'bnp', 'société générale', 'crédit agricole', 'banque populaire', 'caisse épargne', 'lcl', 'frais bancaires', 'commission', 'intérêts'],
...
const HEADER_KEYWORDS = [
  ...
  'numéro de compte', 'iban', 'bic', 'relevé', 'extrait', 'période', 'page',
  'total', 'sous-total', 'récapitulatif', 'résumé', 'balance', 'ancien solde',
  'nouveau solde', 'solde initial', 'solde final', 'solde créditeur', 'solde débiteur',
  'date valeur', 'date d\'opération', 'libellé', 'débit', 'crédit', 'référence',
  'n° de compte', 'titulaire', 'agence', 'code guichet', 'intérêts', 'frais',
  'commission', 'taux', 'plafond', 'limite', 'échéance', 'prélèvement automatique',
  'conditions', 'tarif', 'barème', 'en notre faveur', 'en votre faveur',
  'au en notre', 'jusqu\'au', 'aturation', 'maturation', 'encours',
];
...
export function isReasonableTransactionAmount(amount: number): boolean {
  ...
  if (absAmount < 0.50) return false;
  return true;
}
...
export async function parsePdfAndExtract(buffer: Buffer, yearId?: number | null): Promise<{
  ...
  const transactionsWithSuggestions = parsedTransactions.map(t => {
    ...
    const descSuggestion = !suggestion && t.thirdParty 
      ? suggestCategory(t.description, items) 
      : null;
