...
router.post('/years', asyncHandler(async (req, res) => {
...
router.put('/years/:id', asyncHandler(async (req, res) => {
...
router.put('/groups/reorder', asyncHandler(async (req, res) => {
...
router.get('/items/unassigned', asyncHandler(async (_req, res) => {
  const currentYear = new Date().getFullYear();
...
router.put('/items/move', asyncHandler(async (req, res) => {
...
router.put('/items/reorder', asyncHandler(async (req, res) => {
...
router.put('/items/:itemId/months/:month', asyncHandler(async (req, res) => {
  const itemId = parseInt(req.params.itemId, 10);
  const month = parseInt(req.params.month, 10);
  const { budget: budgetValue, actual } = req.body;

  if (isNaN(itemId) || isNaN(month)) {
